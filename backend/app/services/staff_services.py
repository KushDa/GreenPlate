...
class StaffService:
    ...
    async def get_staff_details(self, id_token: str):
        try:
    ...
    async def get_staff_me(self, id_token: str):
        staff_data, staff_uid = await self.get_staff_details(id_token)
        if not staff_data:
            return JSONResponse(status_code=status.HTTP_401_UNAUTHORIZED, content={"message": "Unauthorized"})

        stall_name = "Unknown Stall"
        try:
            stall_doc = db.collection("colleges").document(staff_data.get("college_id")).collection("stalls").document(staff_data.get("stall_id")).get()
            if stall_doc.exists:
    ...
    async def activate_staff(self, id_token: str):
    ...
    async def update_staff_profile(self, profile_data: UpdateStaffProfileSchema, id_token: str):
      try:
        staff_data, uid = await self.get_staff_details(id_token)
    ...
    def validate_extracted_items(self, items):
    ...
    async def upload_menu(self, menu_data: MenuSchema, id_token: str):
      try:
        staff_data, staff_uid = await self.get_staff_details(id_token)
    ...
    async def get_menu(self,id_token: str):
      try:
        staff_data, staff_uid = await self.get_staff_details(id_token)
    ...
    async def update_menu_item(self, item_id: str, update_data: UpdateMenuItemSchema, id_token: str):
      try:
        staff_data, staff_uid = await self.get_staff_details(id_token)
    ...
    async def delete_menu_item(self, item_id: str, id_token: str):
      try:
        staff_data, staff_uid = await self.get_staff_details(id_token)
    ...
    def _extract_menu_from_image(self, image_bytes: bytes, mime_type: str) -> list:
    ...
    """
      ...
      return self.validate_extracted_items(raw_items)

    async def scan_menu_image(self, file: UploadFile, id_token: str):
      try:
        staff_data, staff_uid = await self.get_staff_details(id_token)
        ...
        extracted_items = self._extract_menu_from_image(contents, file.content_type)
    ...
    async def get_stall_orders(self, id_token: str, status_filter: str = "PAID"):
      try:
    ...
    async def update_order_status_staff(self, order_id: str, status_data: UpdateOrderStatusSchema, id_token: str):
      try:
        staff_data, _ = await self.get_staff_details(id_token)
        if not staff_data:
          return JSONResponse(status_code=status.HTTP_401_UNAUTHORIZED, content={"message": "Unauthorized"})
        ...
        return JSONResponse(status_code=status.HTTP_200_OK,
                            content={"message": f"Order status updated to {status_data.status}"})

      except Exception as e:
        return JSONResponse(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, content={"message": str(e)})

    async def verify_order_pickup(self, verify_data: VerifyPickupSchema, id_token: str):
      try:
        staff_data, _ = await self.get_staff_details(id_token)
        if not staff_data:
          return JSONResponse(status_code=status.HTTP_401_UNAUTHORIZED, content={"message": "Unauthorized"})
    ...
    async def get_stall_resale_items(self, id_token: str):
      try:
        staff_data, _ = await self.get_staff_details(id_token)
        if not staff_data:
    ...
    async def update_resale_price(self, resale_id: str, new_price: float, id_token: str):
      try:
        staff_data, _ = await self.get_staff_details(id_token)
        if not staff_data:
          return JSONResponse(status_code=401, content={"message": "Unauthorized"})
