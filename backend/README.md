...
docker run --rm -p 8080:8080 --env-file .env -v $(pwd)/secrets:/app/secrets:ro greenplate-backend
```
...
### Menu upload & scan
- Menu upload expects JSON matching `MenuSchema` (see `app/schemas/staff.py`): `stall_id` must match authenticated staff's stall; `items` cannot be empty; `price` must be > 0.
...
- Swagger UI: http://localhost:8080/docs — use the Authorize button and paste the idToken (Bearer token).
...
3. uvicorn app.app:app --reload --host 0.0.0.0 --port 8080
