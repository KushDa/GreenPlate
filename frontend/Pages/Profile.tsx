...
const NameEditor: React.FC<{
  ...
  onSave: () => void;
  onCancel: () => void;
}> = ({ isEditing, currentName, editedName, saving, error, onEdit, onChange, onSave, onCancel }) => {
  if (isEditing) {
    return (
      <div className="space-y-2">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="flex gap-2">
            <button onClick={onSave} disabled={saving} className="p-2 bg-emerald-500 text-white rounded-lg hover:bg-emerald-600 disabled:opacity-50">
              <Check size={16} />
            </button>
            <button onClick={onCancel} disabled={saving} className="p-2 bg-gray-200 text-gray-600 rounded-lg hover:bg-gray-300">
              <X size={16} />
            </button>
          </div>
        </div>
...
const Profile: React.FC = () => {
  ...
  const isStaff = !!(staffProfile || (userRole as string) === 'staff');
  ...
  const saveNameChange = async () => {
    ...
    try {
      ...
      if (isStaff) {
        await api.patch('/staff/profile', { name: trimmed }, { headers: { Authorization: `Bearer ${token}` } });
        if (staffProfile) setStaffProfile({ ...staffProfile, name: trimmed });
      } else {
        await api.patch('/user/profile', { name: trimmed }, { headers: { Authorization: `Bearer ${token}` } });
        setLocalName(trimmed);
      }
      setIsEditingName(false);
    } catch (err) {
      setError((err as ApiError).response?.data?.message || 'Failed to update name');
    } finally {
      setSaving(false);
    }
  };
